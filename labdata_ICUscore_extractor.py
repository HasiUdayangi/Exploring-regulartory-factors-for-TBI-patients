...
class LabDataExtractor:
    ...
    def all_icuscore_extract(self, patient_ids, parameter_ids, admission_date):
        column_mapping = {
            21654: 'GCS(Eyes)',
            21655: 'GCS (Motor)',
            21656: 'GCS (Verbal)',
        ...
        for patient_id in patient_ids:
            query = self.get_icuscore_value([patient_id], parameter_ids)
            df_patient = pd.DataFrame(columns=['time'])
            df_query = self.athena.query_as_pandas(query).drop_duplicates().reset_index(drop=True)
    ...
    def icuscore_extract(self, patient_ids, parameter_ids, admission_date):
        column_mapping = {
            'GCS(Eyes)': 21654,
            'GCS(Motor)': 21655,
            'GCS(Verbal)': 21656,
        ...
        param_text_dict = {21654: {1: "4", 2: "3", 3: "2", 4: "1", 5: "1"},
                           21655: {1: "6", 2: "5", 3: "4", 4: "3", 5: "2", 6: "1"},
                           21656: {1: "5", 2: "4", 3: "3", 4: "2", 5: "1", 6: "5", 7: "3", 8: "1"}
                          }

        df = self.all_icutext_extract(patient_ids, parameter_ids, admission_date)

        for column, param_id in column_mapping.items():
            if param_id in param_text_dict:
                df[column] = df[column].map(param_text_dict[param_id])

        df = df.rename(columns=column_mapping)
        df = df.rename(columns={'21654': 'GCS(Eyes)', '21655':'GCS(Motor)', '21656':'GCS(Verbal)'})
